...
export default function GovLoginPage() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (email && password === '1') {
      // Check if gov is verified
      const isVerified = sessionStorage.getItem('gov_verified') === 'true';
      const pendingVerification = sessionStorage.getItem('gov_pending_verification') === 'true';
      ...
      // Save session
      sessionStorage.setItem('gov_authenticated', 'true');
      sessionStorage.setItem('gov_email', email);
      sessionStorage.setItem('gov_commune', 'Xã Tu Mơ Rông'); // Mock data
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-900 via-indigo-900 to-purple-900 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl p-8 border border-white/20">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 text-sm">Xã Nông Thôn Mới Số</p>
        </div>

        {/* Registered Notice */}
        {showRegisteredNotice && (
          <div className="mb-5 bg-blue-50 border border-blue-200 text-blue-700 px-4 py-3 rounded-lg text-sm">
            <div className="flex items-start gap-2">
              <i className="ri-information-line text-blue-600 text-lg mt-0.5"></i>
              <div>
                <p className="font-semibold mb-1">Đăng ký thành công!</p>
                <p>Hồ sơ của bạn đang được Ban Quản lý VITA Platform xem xét. Bạn sẽ nhận được email khi có kết quả phê duyệt (thường trong 3-5 ngày làm việc).</p>
                <button
                  onClick={() => setShowRegisteredNotice(false)}
                  className="mt-2 text-blue-800 underline text-xs"
                >
                  Đóng thông báo
                </button>
              </div>
        ...
        <form onSubmit={handleLogin}>
          <div className="mb-5">
            ...
            <input
              ...
              required
              className="w-full px-4 py-3 border border-gray-300 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors"
            />
          </div>
          ...
          <button
            type="submit"
            className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-semibold py-3 rounded-lg flex items-center justify-center gap-2 transition-all shadow-lg hover:shadow-xl mb-4"
          >
          ...
          <button 
            type="button"
            className="w-full text-sm text-gray-600 hover:text-blue-600 transition-colors cursor-pointer mb-6"
          >
            Quên mật khẩu?
          </button>
        </form>
        ...
        <div className="text-center text-xs text-gray-500 pt-4 border-t mt-6">
          <p>
            ...
            Cần hỗ trợ? Liên hệ: <span className="text-blue-600 font-medium">support@vita.vn</span>
          </p>
        </div>
      </div>
    </div>
